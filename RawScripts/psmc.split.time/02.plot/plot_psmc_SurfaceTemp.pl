...
#print $fh qq(set title "$title_str";); # the title line
  ...
  set y2ran [-18:14]
  set y2label "Surface air temperature({/Symbol \260}C) or global sea level(m)" $afont rotate by 270;
  set out "$prefix.eps";
  ...
  if ( $opts{M}) {
	  ...
	  print $fh qq(,"bintanja2008.txt.nohead.tab.SeaLevl.txt" u 1:2 t "SeaL" w st ls 100 axes x1y2;\n);
      } else {
