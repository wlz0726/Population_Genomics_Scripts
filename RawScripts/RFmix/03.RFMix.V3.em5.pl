...
";
	print O2 "export PATH=$pythonbin:\$PATH; export PYTHONPATH=$pythonpath:\$PYTHONPATH; $pythonbin/python2.7 $rfmix/RunRFMix.py PopPhased $out/$pop/$pop.$chr.alleles.txt $out/$pop/$pop.$chr.classes.txt 02.seperate.pop.pl.out/$pop/$chr.markers.locations2 -o $out/$pop/$pop.$chr.rfmix -n 5 --succinct-output --skip-check-input-format -e 5;\n";
	print O3 "perl region.pl 02.seperate.pop.pl.out/$pop/$chr.markers $out/$pop/$pop.$chr.rfmix.5.SNPsPerWindow.txt $out/$pop/$pop.$chr.rfmix.5.Viterbi.txt; perl plot.win.2.pl $out/$pop/$pop.$chr.rfmix.5.Viterbi.txt.withPos.gz; \n";
	print O4 "perl plot.win.3.normal.pl $out/$pop/$pop.$chr.rfmix.5.Viterbi.txt.withPos.gz;\n";
	close R;
    }
