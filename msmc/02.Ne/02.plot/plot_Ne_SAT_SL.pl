...
set xlab "Years ago (g=$opts{g}, {/Symbol m}=$ylab_aux)" $afont;
  set t po eps enhance so co "Helvetica,15";
  );
#  my $AADD=$yran/10 ;
  ...
  set y2ran [-20:15]
  set y2label "Tsurf({/Symbol \260}C) or RSL(m) " $afont rotate by 270;
  ...
  set style line 13 lt 13 lc rgb "#8B0000" $lw;
...
if ( $opts{M}) {
    ...
    print $fh qq("bintanja2008.txt.nohead.tab.SurfaceTemp.txt" u 1:2 t "Tsurf" w st ls 101 axes x1y2);
    print $fh qq(,"bintanja2008.txt.nohead.tab.SeaLevl.txt" u 1:2 t "RSL" w st ls 100 axes x1y2);
    print $fh qq(,"$prefix.0.txt" u 1:2 t "$titles[0]" w st ls 1);
